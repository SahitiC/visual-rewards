...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\csahi\\work & study\\probabilistic learning\\visual rewards\\exp1.py',
...
cue_1 = visual.ImageStim(
    win=win,
    name='cue_1', 
    image='cue1.jpg', mask=None,
    ori=0.0, pos=(-0.4, 0), size=(0.3, 0.6),
...
for thisTrial in trials:
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [cue_1, cue_2, key_resp]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *cue_1* updates
        if cue_1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if cue_1.status == STARTED:
            ...
            if tThisFlipGlobal > cue_1.tStartRefresh + 3.0-frameTolerance:
        ...
        if cue_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > cue_2.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
